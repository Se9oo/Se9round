...
// 게시글 등록
router.post('/api/post', verifyTokens, async (req, res) => {
  try {
    if (!req.body) {
...
// 게시글 임시 저장
router.post('/api/post/temp', verifyTokens, async (req, res) => {
  try {
    if (!req.body) {
      res.status(500).json('empty request');
    } else {
...
// 게시글 수정
router.post('/api/post/modify', verifyTokens, (req, res) => {
  try {
    if (!req.body) {
      res.status(500).json('empty request');
    } else {
      ...
      client.query(modifyPost, [title, tagsArray, contents, thumbNail, subTitle, POST_OK_STATUS], (error, response) => {
        if (error) {
          res.status(500).json(error);
        } else {
          res.status(200).json('success');
